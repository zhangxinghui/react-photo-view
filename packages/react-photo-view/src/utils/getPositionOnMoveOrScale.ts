...
export default function getPositionOnMoveOrScale(
  ...
  scale: number,
  toScale: number,
  clientX: number = window.innerWidth / 2,
  clientY: number = window.innerHeight / 2,
  offsetX: number = 0,
  offsetY: number = 0,
) {
  // 是否接触边缘
  const [closedEdgeX] = computePositionEdge(x, toScale, width, window.innerWidth);
  const [closedEdgeY] = computePositionEdge(y, toScale, height, window.innerHeight);
  ...
  // 长图模式无左右反馈
  const longModeEdge = height / width >= longModeRatio && width * toScale === window.innerWidth;
  // 超出边缘距离减半
  return {
